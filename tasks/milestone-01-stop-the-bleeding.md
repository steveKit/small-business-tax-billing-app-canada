...
- **User-side follow-up:** update any host aliases, saved DB-client connections, or scripts still pointing at `localhost:5434` for this project. The implementer's grep confirmed zero remaining `5434` references in the repo outside historical notes in `CLAUDE.md`, `Handoffs/`, and `tasks/`.
- **Discovered:** the compose file still has no top-level `name:` and no pinned `postgres_data` / `tax-billing-network` names — logged as DW-009 in [[tasks/discovered]].
