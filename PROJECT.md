...
TASK-014 (FilePicker-based PDF and backup downloads; fixes the WSLg
xdg_foreign issue; PR #16, `379e6be`). Three Milestone 1 tasks remain —
TASK-003 (strong secrets generation, user-gated data-loss event),
TASK-006 (stale backup file cleanup, user-gated sudo), and TASK-007
(integration verification). Milestone 1 tag awaits TASK-007. **Payment
creation is unblocked** — user can record the real Adamson payments to
close TASK-013's reconciliation loop through the normal UI flow.
**PDF and backup downloads are unblocked** — both now save via native
FilePicker dialogs instead of the broken `launch_url` path. Recommended
dispatch order for remaining M1 work: record real payments → TASK-003
→ TASK-006 → TASK-007.
...
| 1 — Stop the Bleeding | — | `in_progress` | 8/11 tasks shipped (PRs #4, #5, #10, #12, #14, #16); TASK-003/006/007 pending; TASK-013/014/015/016 all landed session 004 |
